...
var listRef = (0, _storage.ref)(storage, "images/");
var ImgList = document.querySelector(".images-container");
(0, _storage.listAll)(listRef).then(function (res) {
  res.items.forEach(function (itemRef) {
    console.log(itemRef);
    (0, _storage.getDownloadURL)(itemRef).then(function (downloadURL) {
      ImgList.insertAdjacentHTML('afterbegin', "\n                        <div class=\"preview-image\">\n                            <img src=\"".concat(downloadURL, "\" alt=\"").concat(itemRef === null || itemRef === void 0 ? void 0 : itemRef.name, "\"/>                       \n                        </div>\n                        "));
    });
  });
}).catch(function (error) {
  console.log(error);
...
(0, _upload.upload)('#file', {
  ...
  onUpload: function onUpload(files, blocks) {
    new Promise(function (resolve) {
      resolve(files.map(function (file, index) {
        var ImgRef = (0, _storage.ref)(storage, "images/".concat(file.name));
        var uploadTask = (0, _storage.uploadBytesResumable)(ImgRef, file);
        uploadTask.on('state_changed', function (snapshot) {
          var progress = (snapshot.bytesTransferred / snapshot.totalBytes * 100).toFixed(0);
          var block = blocks[index].querySelector('.preview-info-progress');
          block.textContent = progress;
          block.style.width = progress + "%"; // console.log('Upload is ' + progress + '% done');
      ...
      console.log(blocks);
      ImgList.innerHTML = "".concat(blocks.join(''));
      (0, _storage.listAll)(listRef).then(function (res) {
        res.items.forEach(function (itemRef) {
          console.log(itemRef);
          (0, _storage.getDownloadURL)(itemRef).then(function (downloadURL) {
